...
const GroupTable = ({ listGroups, fetchGroup, rowAction }: any) => {
    const COLUMNS = [
        {
            Header: "#",
            Cell: (row: any) => {
        ...
        },
        {
            ...
            accessor: (row: any) => {
                return (
                    <div>
                        {
                            row.users.map((item: any, i: number) => (
    ...
    const data = useMemo(() => listGroups?.data, []);
    ...
    const getIndex = (idx: any) => {
        const currentPage = listGroups?.current_page;
        const perPage = listGroups?.per_page;
        if (currentPage == 1) {
            return Number(idx) + 1
        } else {
    ...
    return (
        <>
            ...
            <PaginationButtons
                totalPages={listGroups?.last_page}
                currentPage={listGroups?.current_page}
