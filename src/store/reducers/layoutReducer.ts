...
const initialState = {
  ...
  listView: 'table',
  listPage: 0,
  submitLoading: initialSubmitLoading(),
  listSharedPage: 0,
  listTrashPage: 0,
...
export const layoutSlice = createSlice({
  ...
  reducers: {
    // handle list view
    handleSubmitLoading: (state, action) => {
    ...
    handleListSharedPage: (state, action) => {
      state.listSharedPage = action.payload;
    },
    handleListTrashPage: (state, action) => {
      state.listTrashPage = action.payload;
    },
    // handle dark mode
